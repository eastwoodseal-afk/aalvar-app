...
export default function AuthModal({ onClose }: { onClose: () => void }) {
  ...
  const [username, setUsername] = useState(''); // <-- AÑADE ESTA LÍNEA
...
if (isSignUp) {
  // 1. Creamos el usuario en auth.users PASANDO el username como metadata
  const { data: signUpData, error: signUpError } = await supabase.auth.signUp({
    email,
    password,
    options: {
      data: {
        username: username, // <-- ¡CLAVE! Pasamos el username aquí
      }
    }
  });
  ...
  // 2. ELIMINAMOS la inserción manual, porque ahora lo hará la base de datos
  // if (signUpData.user) { ... }
  ...
  <div>
    ...
    />
        ...
        <div className="mt-6">
          ...
          <button className="mt-4 w-full flex justify-center items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
